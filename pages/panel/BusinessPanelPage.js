function BusinessPanelPage() {
  try {
    ...
    const isVip = plan !== 'básico';

    const modules = [
      { title: 'Perfil', desc: 'Logo, portada, descripcion y redes.', icon: 'icon-store', plans: ['vip', 'básico'], status: 'Listo para conectar' },
      { title: 'Servicios', desc: 'Servicios con precios y duracion.', icon: 'icon-list', plans: ['vip'], status: 'VIP' },
      { title: 'Productos', desc: 'Mini tienda con fotos, stock y precios.', icon: 'icon-shopping-bag', plans: ['vip', 'básico'], status: 'Tienda' },
      { title: 'Cursos', desc: 'Cursos, talleres, modalidad y precio.', icon: 'icon-graduation-cap', plans: ['vip', 'básico'], status: 'Cursos' },
      { title: 'Ubicación', desc: 'Provincia, zona y direccion pública.', icon: 'icon-map-pin', plans: ['vip', 'básico'], status: 'Mapa' },
      { title: 'Pedidos', desc: 'Pedidos enviados por WhatsApp.', icon: 'icon-receipt-text', plans: ['vip', 'básico'], status: 'Proximo' },
      { title: 'Reseñas', desc: 'Valoraciones y comentarios de clientes.', icon: 'icon-star', plans: ['vip'], status: 'VIP' },
      { title: 'Estadisticas', desc: 'Vistas, reservas y rendimiento.', icon: 'icon-chart-no-axes-column', plans: ['vip'], status: 'VIP' }
    ];

    const canUse = (module) => module.plans.includes(isVip ? 'vip' : 'básico');
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5" data-name="panel-hero-row" data-file="pages/panel/BusinessPanelPage.js">
            <div data-name="panel-copy" data-file="pages/panel/BusinessPanelPage.js">
              ...
              <p className="mt-3 text-sm md:text-base text-[var(--text-muted)] max-w-[720px] leading-relaxed" data-name="panel-subtitle" data-file="pages/panel/BusinessPanelPage.js">
                Estructura inicial para que negocios VIP y básicos organicen perfil, tienda, cursos y ubicacion.
              </p>
            </div>
        ...
        <section className="mt-5 surface-rr p-5 md:p-6" data-name="panel-next" data-file="pages/panel/BusinessPanelPage.js">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4" data-name="panel-next-row" data-file="pages/panel/BusinessPanelPage.js">
            <div data-name="panel-next-copy" data-file="pages/panel/BusinessPanelPage.js">
              <p className="text-sm font-semibold" data-name="next-title" data-file="pages/panel/BusinessPanelPage.js">Siguiente conexión</p>
